...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_mask__WEBPACK_IMPORTED_MODULE_3__["default"])('[name="phone"]'); // checkNumInputs('[name="name"]');
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var calc = function calc(size, material, option, promocode, result) {
...
var collapse = function collapse(triggersSelector) {
...
var filter = function filter() {
  ...
  items.forEach(function (item) {
    ...
    item.addEventListener('click', function (e) {
      ...
      var markType = wrapper.querySelectorAll(".".concat(targetClass)); // Отримуємо всі елементи з таким самим класом МЕНЮ
...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var statusImg = document.createElement('img'); //створили спінер завантаження

      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('amimated', 'fadeInUp');
      statusMessage.appendChild(statusImg);
      var textMessage = document.createElement('div'); //створили напис завантаження

      textMessage.textContent = message.loading;
      statusMessage.appendChild(textMessage);
      var formData = new FormData(item); // FormData збирає всі дані з форми.

      var api; // куди будем відправляти форму
      ...
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_7__["postData"])(api, formData).then(function (res) {
        // обробка вдачної відправки форми. 
        console.log(res);
        statusImg.setAttribute('src', message.ok);
        textMessage.textContent = message.success;
      }).catch(function () {
        // обробка помилки при відправці
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    var matrix = '+7 (___) ___ __ __',
        ...
        // значення на основі матриці
    val = this.value.replace(/\D/g, ''); // значення на основі введені користувачем

    if (def.length >= val.length) {
      // заборона користувачю знінювати маску
      val = def;
    }

    this.value = matrix.replace(/./g, function (a) {
      return /[_\d]/.test(a) && i < val.length ? val.charAt(i++) : i >= val.length ? '' : a; // перевіряємо кожний введений символ. Якщо ЦИФРА 'i' меньше введених цифр,  повертаємо наступний символ. Якщо 'i' більше введених цифр, повертаємо ''. При всіх інших повертаємо просто символ
    });

    if (event.type === 'blur') {
      //Якщо фокус вийшов з форми
      if (this.value.length == 2) {
        this.value = '';
      }
    } else {
      //Якщо подія ФОКУС, або ІНПУТ
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  function createCards(response) {
    response.forEach(function (_ref) {
      ...
      card.innerHTML = "\n            <div class=\"styles-block\">\n                <img src=".concat(src, " /img/styles-5.jpg alt>\n                <h4>").concat(title, "</h4>\n                <a href=\"").concat(link, "\">\u041F\u043E\u0434\u0440\u043E\u0431\u043D\u0435\u0435</a>\n            </div>\n            ");
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  } catch (e) {} // Навішуємо обробник на весь слайдер через parentNode, та в paused запамятовуємо інтервал при нведенні миші
